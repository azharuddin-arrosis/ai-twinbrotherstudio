                ...
                {featuredPortfolio.length > 0 && (
                    <section className="mb-10">
                        <div className="flex items-center justify-between mb-5">
                            ...
                            <Link href="/portfolio" className="text-sm text-indigo-600 hover:text-indigo-700 flex items-center gap-1">
                                View all projects <ArrowRight size={13} />
                            </Link>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                            {featuredPortfolio.map((item) => (
                                <Link key={item.id} href={`/portfolio/${item.slug}`} className="group block hover:opacity-90 transition-opacity">
                                    {item.image_path ? (
                                        <img src={'/storage/' + item.image_path} alt={item.title} className="aspect-video object-cover rounded-lg bg-gray-100 w-full" />
                                    ) : (
                                    ...
                                    {item.tech_stack && item.tech_stack.length > 0 && (
                                        <div className="flex flex-wrap gap-1 mt-2">
                                            {item.tech_stack.slice(0, 3).map((tech) => (
                                                <span key={tech} className="text-xs bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full">{tech}</span>
                                            ))}
                                        </div>
                                    )}
                                </Link>
